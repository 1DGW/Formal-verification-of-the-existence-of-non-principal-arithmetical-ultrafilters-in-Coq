...
Corollary PreimageSet_Corollary : ∀ f A, Function f
  -> A ∩ ran(f) <> Φ <-> f⁻¹「A」 <> Φ.
Proof.
  split; intros.
  - intro. elim H0.
...
(* 关于像集和原像集的一些简单性质 *)
Fact ImageSet_Property1 : ∀ f A B, f⁻¹「A ∪ B」 = f⁻¹「A」 ∪ f⁻¹「B」.
Proof.
...
Fact ImageSet_Property2 : ∀ f A B, f⁻¹「A ∩ B」 = f⁻¹「A」 ∩ f⁻¹「B」.
Proof.
  intros. apply AxiomI; split; intros.
...
Fact ImageSet_Property3 : ∀ f A B, f「A ∩ B」 ⊂ (f「A」 ∩ f「B」).
Proof.
...
Fact ImageSet_Property4 : ∀ f A B, f⁻¹「A ~ B」 = f⁻¹「A」 ~ f⁻¹「B」.
Proof.
  intros. apply AxiomI; split; intros.
  - apply AxiomII in H as [H[]]. apply MKT4' in H1 as [].
...
Fact ImageSet_Property5 : ∀ f A B, (f「A」 ~ f「B」) ⊂ f「A ~ B」.
Proof.
...
Fact ImageSet_Property6 : ∀ f A, A ⊂ dom(f) -> A ⊂ f⁻¹「f「A」」.
Proof.
...
Fact ImageSet_Property7 : ∀ f A, Function1_1 f -> A ⊂ dom(f)
  -> A = f⁻¹「f「A」」.
Proof.
...
Fact ImageSet_Property8_a : ∀ f A, f「f⁻¹「A」」 ⊂ A.
Proof.
  intros. unfold Included; intros. apply AxiomII in H as [H[x[]]].
  apply AxiomII in H1 as [H1[]]. rewrite H0; auto.
Qed.

Fact ImageSet_Property8_b : ∀ f A, Function f -> A ⊂ ran(f)
  -> f「f⁻¹「A」」 = A.
Proof.
...
Fact ImageSet_Property9_a : ∀ f, Function f -> f「dom(f)」 = ran(f).
Proof.
...
Fact ImageSet_Property9_b : ∀ f, Function f -> f⁻¹「ran(f)」 = dom(f).
Proof.
...
(* 函数复合的取象集和原象集 *)
Fact ImageSet_Property10_a : ∀ f g A, Function f -> (g ∘ f)「A」 = g「f「A」」.
Proof.
  intros. apply AxiomI; split; intros.
...
Fact ImageSet_Property10_b : ∀ f g A, Function f -> Function g
  -> (f ∘ g)⁻¹「A」 = g⁻¹「f⁻¹「A」」.
Proof.
  intros. apply AxiomI; split; intros.
...
Corollary βA_is_Set : ∀ A, Ensemble (β A).
...
Proof.
  ...
  apply Filter_is_Set_Co1 in H3. destruct H; auto. destruct H1 as [_[H1[]]].
  rewrite H in H4; auto.
Qed.

(* 定义f〈F∣B〉, 是对某个超滤F的变换,满足一定条件时(定理5),
  变换后的f〈F∣B〉是B上的超滤 *)
Definition Transform F f B := \{ λ u, u ⊂ B /\ f⁻¹「u」 ∈ F \}.

Notation "f 〈 F ∣ B 〉" := (Transform F f B)(at level 5) : filter_scope.

(* 任意A上的超滤F可以变换为另一个B上的超滤 *)
...
Proof.
  intros. rewrite FT5_a; auto. apply Fa_Corollary2_b; auto.
  apply H1,(@ Property_ran a),Property_Value; auto. rewrite H0; auto.
Qed.

(* 任意超滤(即便是非主超滤)都可以变换成一个主超滤 *)
...
Proof.
  intros. rewrite (FT6_a F0 f A B b); auto.
  apply Fa_Corollary2_b; auto.
Qed.

(* 根据定理7可以构造常值函数Const, 以便记主超滤为Const〈F〉. *)
Definition Const A b := \{\ λ u v, u ∈ A /\ v = b \}\.

(* 验证 Const确是 A上到b的常值函数 *)
Corollary Const_is_Function : ∀ A b, A <> Φ -> Ensemble b
...
(* 验证 任意一个 超滤 经过Const变换后, 都是b主超滤. *)
Corollary F_Const_Fa : ∀ F0 A B b, F0 ∈ (β A) -> b ∈ B
...
(*在定理FT6的基础上,7中的f不需是严格的常值函数,只要在F0的某一个元素集上取常值即可*)
...
(* Theo : If function f and g are equal F0-almost everywhere,
...
(* Coro : f and g in the Def above can be A → B;
          namely, AUF meets the conversion of FT8   *)
Corollary AUF_Corollary : ∀ f g A B F0, Function f -> Function g
  -> dom(f) = A -> dom(g) = A -> ran(f) ⊂ B -> ran(g) ⊂ B
...
(* Theo : Every principal ultrafilter over an infinite set A is an AUF *)
Theorem FT9 : ∀ A a, Ensemble A -> ~ Finite A -> a ∈ A
  -> Arithmetical_ultraFilter (F A a) A.
Proof.
  intros. assert ((F A a) ∈ (β A)).
  { apply (Fa_Corollary2_b A a) in H; [apply AxiomII; split; auto| ]; auto.
...
(* Lem : Transformation of ultrafilters by composite functions *)
...
(* Theo : AUF over A can be transformed to an AUF over B
...
Proof.
  ...
  apply (AUF_Corollary _ _ A) in H11 as [_[_[_[_[_[_[_]]]]]]]; auto.
