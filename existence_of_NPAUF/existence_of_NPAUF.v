...
(** existence_of_nonprincipal_arithmetical_ultrafilters *)

Require Export arithmetical_ultrafilter.
Require Export filter_extension.
...
Proof.
  ...
  { intros. destruct H6 as [A[H6[]]].
    ...
    { intro. apply (free_ultraFilter_Co1 (f〈F0〉) ω H13) in H10; auto. }
...
Lemma Existence_of_NPAUF_Lemma7 : ∀ B f g, P[B] ≼ ω
...
Proof.
  ...
  pose proof (FilterBase_from_Corollary G).
  apply MKT158 in H12. rewrite H in H12. destruct H9,H12; auto.
  elim (MKT102 P[〈G〉→ᵇ] ω); auto.
Qed.

(* 连续统假设: ω与P[pow(ω)]之间没有其他基数 *)
...
set (h := \{\ λ u v, Ordinal dom(u) /\
  ...
  { intros. destruct (classic (m ∈ dom(r))).
    ...
    { destruct H34. destruct H34. rewrite H36,H35 in H28.
      ...
      { rewrite H28. apply FilterBase_from_Corollary. }
  ...
  { intros. apply NNPP; intro.
    ...
    pose proof (FilterBase_from_Corollary μ).
    assert (μ = 〈μ〉→ᵇ).
  ...
  { intros. apply NNPP; intro.
    ...
    { apply NNPP; intro. apply MKT43 in H37.
      ...
      { rewrite H36. apply AxiomI; split; intros. apply MKT19;
        eauto. apply FilterBase_from_Corollary; auto. }
  ...
  { pose proof H3. apply (@ MKT109 dom(r)) in H20 as []; auto.
    ...
    apply Filter_is_Set_Co2 in H23. rewrite H22 in H23.
    elim MKT39; eauto.
    assert (dom(r) ∈ R).
  ...
  { intros. destruct (classic (n ∈ ω)). apply FT2_a in H26 as [_]; eauto.
    ...
    rewrite <-(MKT126c r (P[pow(ω)])); auto.
    apply (@ Property_ran Φ),Property_Value; auto.
    apply Fa_Corollary1 in H26. intro. rewrite H27,H26 in H25.
  ...
  split; intros. apply βA_Corollary1; auto.
  ...
  split; auto. split; auto. repeat split; auto.
  apply βA_Corollary1; auto. pose proof H36. rewrite H5 in H40.
  ...
  apply FilterBase_from_Corollary. apply MKT4; auto.
  ...
  rewrite H41,H38,H39; auto. apply βA_Corollary1; auto.
  ...
  apply βA_Corollary1; auto. exists (c[\{ λ w, w ⊂ ω
    /\ Finite_Intersection (r[n] ∪ [w]) /\ f「w」 ∩ g「w」 = Φ \}]).
  split; auto. apply (H26 (PlusOne n)); auto. rewrite H52.
  apply FilterBase_from_Corollary,MKT4; auto. destruct H49.
